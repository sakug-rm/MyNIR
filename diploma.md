...
Проведённый эксперимент позволяет сформулировать результат подраздела в завершённой форме.
...
Именно в этом виде результат подраздела становится достаточно зрелым для перехода к следующему этапу исследования: выбор инструмента должен быть режимно-зависимым, а сама регрессия должна рассматриваться как часть более общей адаптивной схемы анализа нелинейного процесса.
...
Проведённый эксперимент позволяет сформулировать результат подраздела 2.4.2 в завершённой форме.
...
Тем самым подраздел 2.4.2 естественно продолжает логику подраздела 2.4.1. В подразделе 2.4.1 было показано, что нужно различать инструменты локальной структурной идентификации и устойчивой параметрической оценки. В настоящем подразделе показано, что даже внутри регрессионного контура выбор рабочей конфигурации зависит от режима процесса. Это напрямую подводит к следующему шагу: построению алгоритма предварительной диагностики режима до запуска регрессии.
...
Проведённый эксперимент позволяет сформулировать результат подраздела 2.4.3 в завершённой форме.
...
Тем самым результат подраздела 2.4.3 приближает главу к той схеме, которую требовал научный руководитель: анализ нелинейного процесса должен быть режимно-адаптивным. Регрессия здесь выступает уже не универсальным молотком, а одним из инструментов в более общей архитектуре исследования.
...
Проведённый эксперимент позволяет сформулировать результат подраздела 2.4.4 в завершённой форме.
...
Подраздел 2.4.4 закрывает важный методический пробел главы. После подразделов 2.4.1-2.4.3 уже было ясно, что нужно:
...
Настоящий подраздел добавляет к этому ещё одно правило:
...
Именно в этом состоит основной методологический вклад подраздела 2.4.4. Он показывает, что даже внутри уже выбранного регрессионного контура интерпретация коэффициентов должна быть режимно-адаптивной и учитывать масштабную несопоставимость регрессоров.
...
Подраздел 2.4.5 усиливает предыдущие результаты не новой моделью, а новым уровнем режимной диагностики. После подразделов 2.4.2-2.4.4 уже было ясно, что короткое окно может давать ложные лаги и что интерпретация коэффициентов зависит от режима. Настоящий подраздел переводит это в более точное правило:
...
Тем самым подраздел 2.4.5 добавляет к алгоритму анализа ещё один обязательный шаг:

1. оценить локальную дисперсию $Var(\omega)$;
2. оценить стандартизованную кондиционность $\kappa(\widetilde{X})$;
3. только после этого трактовать lag-структуру и коэффициенты.
...
Тем самым подраздел 2.4.6 непосредственно встраивается в уже полученный режимно-адаптивный алгоритм анализа:
...
Проведённый контрольный эксперимент позволяет зафиксировать итог подраздела 2.4.6 в завершённой форме.
...
Тем самым подраздел 2.4.6 не опровергает ранее построенную архитектуру анализа, а усиливает её. Он показывает, что никакая отдельная техника отбора переменных не может заменить режимно-зависимый маршрут исследования. Регуляризация полезна как дополнительный контрольный слой, но не как самостоятельное решение.
...
Полученный результат важно встроить в общую схему анализа коротких окон. Подраздел 2.4.7 не предлагает новый основной способ чтения лаговой структуры, а уточняет, в каком месте маршрута полезна ортогонализация.
...
Тем самым подраздел 2.4.7 логически не конкурирует с ранее построенной архитектурой, а усиливает её. Он показывает, что даже после подтверждения маломерности окна переход в ортогональные координаты остаётся лишь вспомогательным шагом.
...
Поэтому итоговый вывод подраздела 2.4.7 формулируется так:
...
Дополнительно подраздел 2.4.8 был сознательно связан с подразделом 2.4.5. Каждое окно предварительно относилось к одному из трёх классов интерпретируемости:
...
Тем самым подраздел 2.4.8 проверяет не просто "что делает $\alpha$", а "может ли настройка $\alpha$ заменить уже найденный критерий интерпретируемости". Именно в этом состоит его методический смысл.
...
Гипотеза подраздела 2.4.8 подтверждена.
...
Итоговый вывод подраздела 2.4.8 можно сформулировать так:

> $p$-value в Stepwise является только регулятором жёсткости отбора, но не самостоятельным критерием интерпретируемости и не заменой режимной диагностики. Поэтому настройка $\alpha$ имеет смысл только после прохождения окна через процедуру, заданную подразделами 2.4.3 и 2.4.5; вне этой процедуры она не решает проблему ложных лагов.

Перед переходом к общему заключению итог серии дополнительных экспериментов можно выразить в виде короткой рабочей схемы. Если анализируется короткое окно, то последовательность действий должна быть следующей:

1. сначала диагностируется режим процесса;
2. затем проверяется интерпретируемость окна по локальной дисперсии и кондиционности;
3. после этого выбирается регрессионный или альтернативный контур анализа;
4. и только на последнем шаге интерпретируются лаговая структура, $\beta$-коэффициенты, $B_\Sigma$ и результаты отбора переменных.

Именно такая последовательность и является главным результатом подразделов 2.4.1-2.4.8 как единой исследовательской программы.
