...
## 3.6. Общие выводы по главе реальных данных и практическая значимость
...
### 3.6.3. Ограничения и итоговый вывод
...
## Приложение А. Дополнительные synthetic-эксперименты и проверки устойчивости
...
1. Полные серии однотипных графиков по базовой, запаздывающей и смешанной моделям, включая дополнительные режимы, фазовые портреты и прогнозные продолжения.
2. Резервные parameter sweeps и дополнительные сценарии по параметрам $q$ и $\gamma$ смешанной модели.
3. Развёрнутые robustness checks по длине окна, сравнению $B$ и $\beta$, критерию интерпретируемости, регуляризации, PCA и настройке $p$-порога.
4. Полные таблицы метрик по методам, окнам и режимам, которые были использованы при сжатии раздела 2.4 до основной бакалаврской версии.

Электронные материалы приложения А соответствуют следующим наборам файлов:

- `outputs/planA` — direct vs regression, шумовой sweep и диагностическая полезность;
- `outputs/planB` — влияние длины окна и отношение $W/T$;
- `outputs/planC` — сравнение коэффициентов $B$ и $\beta$;
- `outputs/planD` — ENTER, STEPWISE и регуляризованные методы;
- `outputs/planE` — чувствительность к $p$-порогу;
- `outputs/planF` — PCA и ортогонализация лагового пространства;
- `outputs/planG` — режимная предклассификация и выбор контура;
- `outputs/planH` — дисперсия, кондиционность и риск вырождения окна.

## Приложение Б. Расширенная линия промышленных индексов

В основной текст глава 3 включает только краткую внешнюю проверку переносимости метода на промышленных индексах. Развёрнутые результаты по `IPP` вынесены в приложение Б.

В приложение Б входят:

1. Подробные сравнения трёх слоёв наблюдения (`adj_smoothed`, `adj_unsmoothed`, `raw`).
2. Полные таблицы режимов и интерпретируемости для всех окон по промышленному корпусу.
3. Отраслевые карточки, `case overview` и дополнительные графики по шокам `2020` и `2022`.
4. Расширенные таблицы устойчивости маршрута анализа по промышленным рядам.

Электронные материалы приложения Б соответствуют следующим наборам файлов:

- `outputs/real_data/ipp_routing_all_variants` — режимы, интерпретируемость и выбор рабочего слоя наблюдения;
- `outputs/real_data/ipp_case_cards` — расширенные отраслевые карточки, case overview и шоковые сводки.

## Приложение В. Дополнительные таблицы по линии выручки
...
1. Полные таблицы состава групп фирм по темпам роста и соответствующие годовые медианные траектории.
2. Развёрнутые interval- и window-summary таблицы по группам выручки.
3. Полные секторные таблицы по отраслевому подкорпусу выручки, включая покрытия, устойчивость и шоковые профили.

Электронные материалы приложения В соответствуют следующим наборам файлов:

- `outputs/real_data/revenue_b1` — группы фирм по темпам роста, интервальные и оконные результаты;
- `outputs/real_data/revenue_b2` — отраслевой подкорпус выручки, карты читаемости и шоковые сводки.
